...
var http = require('http'),
    ...
    Q = require('q'),
    cache = require('./cache'),
    debug = require('debug')('requestify'),
    https = require('https'),
...
var Requestify = (function() {
    ...
    /**
     * Returns http|s agent instance according to the given protocol
     * @param protocol
     * @returns {http|https}
     */
     ...
     */
    function storeCache(url, code, headers, body, cacheSettings, created, buffered) {
        debug("caching request - "+ url);
        debug("cache settings - " + JSON.stringify(cacheSettings));
        debug(code + "\n" + headers + "\n" + body + "\n" + created + "\n" + buffered);
    ...
    function call(request, defer) {
        var httpRequest,
            ...
            agent = getAgent(request.getProtocol()),
        ...
        httpRequest = http.request(options, function(res) {
            ...
            res.on('end', function() {
                ...
                response.responseTime = (diff[0] * NS_PER_SEC + diff[1]) / 1000000; // in milliseconds
        ...
        // httpRequest.on('socket', function (res) {
...
function isRedirect(statusCode) {
    ...
    function callRouter(request) {
        debug("fetch request for " + request.getFullUrl())
        var defer = Q.defer();
        ...
        function bufferExpirationTime(data) {
            return data.created + request.cache.buffer;
        }
        ...
        cache.get(request.getFullUrl())
            .then(function(data) {
                ...
                if(!data.buffered && bufferExpirationTime(data) <= new Date().getTime()) {
                    debug('cache has expired!');
                    storeCache(
                        request.getFullUrl(), data.code, data.headers, data.body,
                        request.cache, data.created, true
                    );

                    call(request, Q.defer());
                }
                defer.resolve(new Response(data.code, data.headers, data.body));
            })
            .fail(function() {
                debug("failed on cache! requesting again.");
                call(request, defer);
            }
        );
